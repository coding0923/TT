...
public class ClassController {
    ...
    @GetMapping("/classs/classstudentexist")
    ...
    public void classdetails(ClassDTO dto, Model model, HttpSession session) {
        ...
        if (session.getAttribute("role") == "student") {
            ...
            attendeedto.setStudentId(studentdto.getStudentId());
            attendeedto.setAcademyId(classdetails.academyId);
            attendeedto.setClassId(classdetails.classId);
            int result = classService.countAttendee(attendeedto);
            if (result == 1) {
                session.setAttribute("attendee", "true");
            } else {
                session.setAttribute("attendee", "false");
            }
        }
    ...
    @PostMapping("/classs/classdetails")
    public String insertAttendee(AttendeeDTO dto, RedirectAttributes rttr) {
        int result = attendeeService.Attendeeput(dto);
        if (result == 1) {
            rttr.addFlashAttribute("attendeeInsert", "succ");
            return "redirect:/classs/classlist";
        } else {
            rttr.addFlashAttribute("attendeeInsert", "fail");
            return "redirect:/classs/classlist";
        }
    }
